--DECLARE @BeginDate DATETIME = '2019-10-29'
--DECLARE @EndDate DATETIME = '2019-10-31'
--DECLARE @FILIAIS INT = 2
...
SELECT		 
	...
	,CASE 
		WHEN GLGV_MOTIVOOCORRENCIAS.TIPO = 1 THEN COUNT(1)
	ELSE 0
	END AS TOTAL_AVARIA
	,CASE 
		WHEN GLGV_MOTIVOOCORRENCIAS.TIPO = 2 THEN COUNT(1)
	ELSE 0
	END AS TOTAL_FALTA
	,CASE 
		WHEN GLGV_MOTIVOOCORRENCIAS.TIPO = 3 THEN COUNT(1)
	ELSE 0
	END AS TOTAL_SOBRA
    --,(
    --       SELECT COUNT(1)
...
LEFT JOIN	GLGV_PLANOCARGADESCARGAS	PLANO			ON PROCESSO.PLANOCARGADESCARGA   = PLANO.HANDLE
LEFT JOIN	GLOP_VIAGENS				VIA				ON PLANO.VIAGEM                  = VIA.HANDLE
...
LEFT JOIN	FILIAIS										ON PROCESSO.FILIAL               = FILIAIS.HANDLE
LEFT JOIN  FILIAIS						FILIALORIGEM	ON FILIALORIGEM.HANDLE			 = VIA.FILIALORIGEM
INNER JOIN	VW_GESTAOVOLUMEVALORES		VALORES			ON VALORES.PROCESSO				 = PROCESSO.HANDLE
LEFT JOIN GLGV_OCORRENCIAS ON GLGV_OCORRENCIAS.PROCESSO = PROCESSO.HANDLE
LEFT JOIN GLGV_MOTIVOOCORRENCIAS ON GLGV_OCORRENCIAS.MOTIVOOCORRENCIA = GLGV_MOTIVOOCORRENCIAS.HANDLE


WHERE		PROCESSO.DATAABERTURA BETWEEN @BeginDate AND @EndDate
AND			PROCESSO.STATUS		= 1335
AND         PROCESSO.FILIAL IN (@FILIAIS)
