...
Select TB1.HANDLE,
  ...
  From (Select FILIAIS.HANDLE,
		 ...
		 Where GLGL_DOCUMENTOS.STATUS							Not In (224, 404) --ok
		   ...
		   And GLGL_DOCUMENTOS.TIPODOCUMENTO					In (1, 2, 17, 22) --ok  
		   And FILIAIS.HANDLE									In (Select FILIAIS.HANDLE
																	  From FILIAIS
																	 Inner Join GLGL_FILIAIS	On GLGL_FILIAIS.FILIAL	= FILIAIS.HANDLE
																	 Where FILIAIS.EMPRESA								= 1
...
Select *,
	   round(IIF(TB2.CATEGORIA	= 'Frete Total',   IsNull( (Select 
																	case
																		when Cast(GLGL_DOCUMENTOS.DATAEMISSAO As Date) > CAST(DATEADD(DD, -1, GETDATE()) AS DATE) then 0
																	else Sum(GLGL_DOCUMENTOS.VALORCONTABIL) 
																	END as	[TOTAL]
														  From GLGL_DOCUMENTOS
														 Inner Join GLGL_FILIAIS	On GLGL_FILIAIS.HANDLE				= GLGL_DOCUMENTOS.FILIAL
														 Inner Join FILIAIS			On FILIAIS.HANDLE					= GLGL_FILIAIS.FILIAL
														 Where GLGL_DOCUMENTOS.STATUS									Not In (224, 404)
														   And GLGL_DOCUMENTOS.STATUS									Not In (220, 223, 236, 237, 417,416,419,421)
  ...
  From (Select HANDLE,
			   ...
			   round(Lead(Valor, 1, 0) Over (Partition By HANDLE, NOME, Categoria, DatePart(Day, Data), DatePart(Month, Data)
										   Order By HANDLE, NOME, Categoria, DatePart(Day, Data), DatePart(Month, Data), DatePart(Year, Data) Desc),2)													[ValorTotalAnt],
