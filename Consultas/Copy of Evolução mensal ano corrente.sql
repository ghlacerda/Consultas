...
Case Month(CT_LANCAMENTOS.COMPETENCIA)
 ...
 Where 1=1 
   ...
   and CAST(FN_DOCUMENTOS.DATAEMISSAO AS DATE) >= CAST(DateAdd(yyyy, DateDiff(yyyy,0,GetDate()), 0) AS DATE)
   and CAST(FN_DOCUMENTOS.DATAEMISSAO AS DATE) < CAST(getdate() AS DATE)
