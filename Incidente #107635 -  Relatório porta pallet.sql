DECLARE @DATA DATETIME = GETDATE()
  ...
  INNER JOIN GLOP_SERVICOSREALIZADOS SERVICO ON SERVICO.HANDLE = SERVICORPS.SERVICOREALIZADO
  INNER JOIN GLGL_SERVICOLOGISTICA SERVICOLOGISTICA ON SERVICOLOGISTICA.HANDLE = SERVICO.SERVICO  AND SERVICOLOGISTICA.HANDLE IN (56,58)
   ...
   INNER JOIN GLGL_DOCUMENTOS NFSE 
		...
		--AND NFSE.STATUS NOT IN (236,237) 
 ...
 WHERE 1=1
 --AND PD.FILIAL = 2 
 AND PD.DATAENTRADA >= CONVERT(VARCHAR, @DATA - DAY(@DATA) + 1, 103)
