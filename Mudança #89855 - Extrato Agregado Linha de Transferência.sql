  ...
  DROP TABLE IF EXISTS #TempViagensMistas
    ...
    SELECT VIAGENS.NUMEROVIAGEM  AS NUMEROVIAGEM,
           ...
           (SELECT SUM(CASE WHEN CONTRATOFRETES.VALORTOTAL IS NULL THEN 0 ELSE CONTRATOFRETES.VALORTOTAL END)
              FROM GLOP_CONTRATOFRETEVIAGENS CONTRATOFRETEVIAGENS WITH(NOLOCK) INNER
                   JOIN GLOP_CONTRATOFRETES CONTRATOFRETES WITH(NOLOCK) ON (CONTRATOFRETEVIAGENS.CONTRATOFRETE = CONTRATOFRETES.HANDLE)
             WHERE CONTRATOFRETEVIAGENS.VIAGEM = VIAGENS.HANDLE
           ...
           ((SELECT SUM(VIAGEMDOCUMENTOS.VALORFRETE)
              ...
              WHERE VIAGEMDOCUMENTOS.VIAGEM = VIAGENS.HANDLE) / NULLIF((SELECT SUM(VIAGEMDOCUMENTOS.PESOCUBADO)
                ...
                FROM GLOP_CONTRATOFRETEVIAGENS CFVG WITH(NOLOCK) INNER
               JOIN GLOP_CONTRATOFRETES CF WITH(NOLOCK) ON (CFVG.CONTRATOFRETE = CF.HANDLE)
		   ...
		   CALC.TARIFAADD,
		   ISNULL(T5.OBSERVACAO,'') AS OBSERVACAO,
		   VB.OBSERVACAOAUTORIZACAO
		   
	  INTO #TempAcompanhamentoViagens
      FROM GLOP_VIAGENS VIAGENS
           INNER JOIN FILIAIS FILIALORIGEM WITH(NOLOCK) ON FILIALORIGEM.HANDLE = VIAGENS.FILIALORIGEM
           INNER JOIN FILIAIS FILIALDESTINO WITH(NOLOCK) ON FILIALDESTINO.HANDLE = VIAGENS.FILIALDESTINO
           INNER JOIN GN_PESSOAS MOTORISTA WITH(NOLOCK) ON MOTORISTA.HANDLE = VIAGENS.MOTORISTA
           INNER JOIN MA_RECURSOS VEICULO1 WITH(NOLOCK) ON VEICULO1.HANDLE = VIAGENS.VEICULO1
           INNER JOIN MF_VEICULOTIPOS VEICULOTIPO WITH(NOLOCK) ON VEICULOTIPO.HANDLE = VEICULO1.TIPOVEICULO
           INNER JOIN GN_PESSOAS BENEFICIARIO WITH(NOLOCK) ON BENEFICIARIO.HANDLE = VEICULO1.PROPRIETARIO
           INNER JOIN GLOP_LINHAVIAGENS LINHAVIAGENS WITH(NOLOCK) ON LINHAVIAGENS.HANDLE = VIAGENS.LINHAVIAGEM
           LEFT JOIN MA_RECURSOS VEICULO2 WITH(NOLOCK) ON VEICULO2.HANDLE = VIAGENS.VEICULO2
           LEFT JOIN MA_RECURSOS VEICULO3 WITH(NOLOCK) ON VEICULO3.HANDLE = VIAGENS.VEICULO3
           INNER JOIN GLGL_ENUMERACAOITEMS TIPOTRANSFERENCIA WITH(NOLOCK) ON TIPOTRANSFERENCIA.HANDLE = VIAGENS.TIPOVIAGEM
           INNER JOIN GLOP_VIAGEMDOCUMENTOS VDOC WITH(NOLOCK) ON (VDOC.VIAGEM = VIAGENS.HANDLE)
		INNER JOIN  (
						SELECT 186 AS TIPO,HANDLE, DOCCLIATUALPESOCONSIDERADO PESOCONSIDERADO FROM GLGL_DOCUMENTOS WITH(NOLOCK)
						WHERE TIPODOCUMENTO = 6
						UNION ALL
						SELECT 188 AS TIPO,HANDLE, DOCCLIATUALPESOCONSIDERADO PESOCONSIDERADO FROM GLGL_DOCUMENTOS WITH(NOLOCK)
		...
		LEFT JOIN GLOP_CONTRATOFRETEVIAGENS VFRETE WITH(NOLOCK)     
		ON VFRETE.VIAGEM = VIAGENS.Handle

		LEFT JOIN K_GLOP_VIAGEMBLOQUEIOS VB WITH(NOLOCK)
	    ON VB.VIAGEM = VIAGENS.HANDLE  
...
FROM   GLOP_CONTRATODESPESAS A WITH(NOLOCK)
       INNER JOIN GLGL_ENUMERACAOITEMS STATUS WITH(NOLOCK) ON A.STATUS = STATUS.HANDLE
       LEFT JOIN GLGL_TIPOLANCAMENTOS TIPOLANC WITH(NOLOCK) ON TIPOLANC.HANDLE = A.TIPOLANCAMENTO
       LEFT JOIN GN_PESSOAS C WITH(NOLOCK) ON A.BENEFICIARIO = C.HANDLE
       LEFT JOIN GLGL_PESSOAS C1 WITH(NOLOCK) ON C1.PESSOA = C.HANDLE
       LEFT JOIN GLGL_ENUMERACAOITEMS RNTRC WITH(NOLOCK) ON C1.CLASSIFICACAORNTRC = RNTRC.HANDLE
       LEFT JOIN FILIAIS FILIAL WITH(NOLOCK) ON A.FILIAL = FILIAL.HANDLE
       LEFT JOIN GLGL_FILIAIS FLGL WITH(NOLOCK) ON FLGL.FILIAL = FILIAL.HANDLE
       LEFT JOIN GN_PESSOAS E WITH(NOLOCK) ON A.MOTORISTA = E.HANDLE
       LEFT JOIN MA_RECURSOS V WITH(NOLOCK) ON A.VEICULO = V.HANDLE
       LEFT JOIN GLOP_VIAGENS VG WITH(NOLOCK) ON A.VIAGEM = VG.HANDLE
       LEFT JOIN GLOP_CIOTTACAGREGADO CIOT WITH(NOLOCK) ON A.CIOT = CIOT.HANDLE
       LEFT JOIN GLOP_CONTRATODESPVENCTOS VENCIMENTOS WITH(NOLOCK) ON VENCIMENTOS.CONTRATODESPESA = A.HANDLE
       LEFT JOIN GLOP_CONTRATODESPORDENS DESPORD WITH(NOLOCK)
       INNER JOIN MF_ORDEMSERVICOS ORDEMSERVICO WITH(NOLOCK) ON ORDEMSERVICO.HANDLE = DESPORD.ORDEMSERVICO
       INNER JOIN MF_TIPOORDEMSERVICOS TIPOORDEM WITH(NOLOCK) ON ORDEMSERVICO.TIPOORDEMSERVICO = TIPOORDEM.HANDLE ON DESPORD.CONTRATODESPESA = A.HANDLE
...
FROM    GLMF_OSVIAGEMCF OSCF WITH(NOLOCK)
INNER JOIN GLOP_CONTRATOFRETEADTDSPS ADT WITH(NOLOCK) ON OSCF.ADIANTAMENTODESPESA = ADT.HANDLE
INNER JOIN GLGL_TIPOLANCAMENTOS TPLANC WITH(NOLOCK) ON ADT.TIPOLANCAMENTO = TPLANC.HANDLE
INNER JOIN GLOP_CONTRATOFRETES CF WITH(NOLOCK) ON OSCF.CONTRATOFRETE = CF.HANDLE
INNER JOIN GLGL_ENUMERACAOITEMS STATUS WITH(NOLOCK) ON CF.STATUS = STATUS.HANDLE
INNER JOIN MF_ORDEMSERVICOS OS WITH(NOLOCK) ON OS.HANDLE = OSCF.ORDEMSERVICO
INNER JOIN MF_TIPOORDEMSERVICOS TPOS WITH(NOLOCK) ON OS.TIPOORDEMSERVICO = TPOS.HANDLE
INNER JOIN GN_PESSOAS C WITH(NOLOCK) ON CF.BENEFICIARIO = C.HANDLE
INNER JOIN GLGL_PESSOAS C1 WITH(NOLOCK) ON C1.PESSOA = C.HANDLE
INNER JOIN GLGL_ENUMERACAOITEMS RNTRC WITH(NOLOCK) ON C1.CLASSIFICACAORNTRC = RNTRC.HANDLE
LEFT  JOIN GN_PESSOAS E WITH(NOLOCK) ON CF.MOTORISTA = E.HANDLE
INNER JOIN MA_RECURSOS V WITH(NOLOCK) ON CF.VEICULO = V.HANDLE
INNER JOIN FILIAIS FILIAL WITH(NOLOCK) ON CF.FILIAL = FILIAL.HANDLE
INNER JOIN GLGL_FILIAIS FLGL WITH(NOLOCK) ON FLGL.FILIAL = FILIAL.HANDLE
LEFT JOIN GLOP_CONTRATODESPESAS WITH(NOLOCK) On GLOP_CONTRATODESPESAS.HANDLE = ADT.CONTRATODESPESA --Adicionado por marcusoliveira conforme chamado 50652 passado pelo GiovanniCampos
...
SELECT DISTINCT  
	...
	,CALC.TARIFAADD AS FRETECOMBINADO
	,0 AS VALORBRUTO
...
FROM GLOP_VIAGENS VIAGENS WITH(NOLOCK)
INNER JOIN FILIAIS FILIALORIGEM WITH(NOLOCK) ON FILIALORIGEM.HANDLE = VIAGENS.FILIALORIGEM
INNER JOIN GLOP_VIAGEMDOCUMENTOS VIAGEMDOCUMENTOS WITH(NOLOCK)
   ON VIAGEMDOCUMENTOS.VIAGEM = VIAGENS.HANDLE
INNER JOIN MA_RECURSOS VEICULO1 WITH(NOLOCK) ON VEICULO1.HANDLE = VIAGENS.VEICULO1
INNER JOIN GLOP_CONTRATOFRETEVIAGENS VFRETE  WITH(NOLOCK)     
	ON VFRETE.VIAGEM = VIAGENS.Handle
INNER JOIN GLOP_CONTRATOFRETES FRETE   WITH(NOLOCK)            
	ON FRETE.HANDLE = VFRETE.CONTRATOFRETE  
LEFT JOIN MA_RECURSOS VEICULO2 WITH(NOLOCK) ON VEICULO2.HANDLE = VIAGENS.VEICULO2
LEFT JOIN MA_RECURSOS VEICULO3 WITH(NOLOCK) ON VEICULO3.HANDLE = VIAGENS.VEICULO3
INNER JOIN MF_VEICULOTIPOS VEICULOTIPO WITH(NOLOCK) ON VEICULOTIPO.HANDLE = VEICULO1.TIPOVEICULO
LEFT JOIN GN_PESSOAS MOTORISTA WITH(NOLOCK) ON MOTORISTA.HANDLE = VIAGENS.MOTORISTA
LEFT JOIN GN_PESSOAS BENEFICIARIO WITH(NOLOCK) ON BENEFICIARIO.HANDLE = VEICULO1.PROPRIETARIO
LEFT JOIN K_GLOP_VIAGEMBLOQUEIOS VB WITH(NOLOCK)
	ON VB.VIAGEM = VIAGENS.HANDLE  
LEFT JOIN 
			(
...
SELECT  DISTINCT 
	...
	,DATAPREVISAOSAIDA
	,ROUND(PESOREAL,2)
	,ROUND(VALORTOTALMERCADORIA,2)
	,ROUND(KMTOTAL,2)
	,ROUND(VALORPORKMLV,2)
	,ROUND(VALORTRANSFERENCIA,2)
	,ROUND(SUM([Valor Distribuição]),2) AS [Valor Distribuição]
	,ROUND(FRETECOMBINADO,2)
	,ROUND(VALORBRUTO,2)
	,ROUND(SUM([Valor Descontos]),2) AS [Valor Despesas]
	--,VENCIMENTO
	,TIPOLANCAMENTO
	,OBSERVACAO
...
LEFT JOIN #TempDespesas B ON A.PLACA = B.VEICULO
