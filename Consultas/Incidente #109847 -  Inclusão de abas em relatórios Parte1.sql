declare @DATAINCLUSAOINICIO datetime = '2019-11-01'
declare @DATAINCLUSAOFIM datetime = '2019-11-02'
...
SELECT OCO.NOME, DOC.DOCUMENTODIGITADO, MOV.DATAINCLUSAO, USU.NOME, MOV.ABATIMENTO, MOV.HISTORICO, PES.NOME, PES.CGCCPF, CAT.nome PoloAtendimento,P.nome GerenteContas,
 ...
 END MOTIVO
 ,F.NOME AS FILIAL
FROM FN_MOVIMENTACOES MOV 
INNER JOIN Z_GRUPOUSUARIOS USU ON USU.HANDLE = MOV.USUARIOINCLUIU 
INNER JOIN GN_PESSOAS PES ON PES.HANDLE = MOV.PESSOA 
INNER JOIN gn_pessoas P on (P.handle = PES.AGENTEVENDAS) 
INNER JOIN GN_CATEGORIASCLIENTE CAT on cat.handle = PES.CATEGORIACLIENTE 
INNER JOIN FN_OCORRENCIAS OCO ON MOV.OCORRENCIA = OCO.HANDLE 
INNER JOIN FN_DOCUMENTOS DOC ON DOC.HANDLE = MOV.DOCUMENTO
INNER JOIN GLGL_LOCALIDADES L ON L.MUNICIPIO = PES.MUNICIPIO AND L.TIPO= 165
INNER JOIN GLGL_LOCALIDADEITEMS LI ON L.HANDLE = LI.LOCALIDADE
INNER JOIN GLGL_LOCALIDADES L2 ON L2.HANDLE = LI.AGLUTINADOR
INNER JOIN GLOP_REGIAOATENDIMENTOS RA ON L2.HANDLE = RA.LOCALIDADE AND RA.STATUS = 566
INNER JOIN FILIAIS F ON RA.FILIAL = F.HANDLE 
