...
--SUBSELECT PARA COMAR TUDO 
		...
		CAST(SUM(ISNULL([PESOCONSIDERADO],0)) AS NUMERIC(10,2)) AS [Peso Considerado], 		
		...
		ISNULL([QUANTIDADEENTREGAREALIZADA],0) AS [Quantidade Entrega Realizada],		
		...
		ISNULL(ROUND((SUM(NULLIF([PESOCONSIDERADO],0))/NULLIF([CAPACIDADE],0)),2),0) AS [Ocupação %],
		...
		ROUND(SUM(ISNULL((NULLIF(VALORCONTRATOFRETE,0)/NULLIF([TOTALFRETET2],TOTALFRETED)),0)),2) AS [Custo Total por Receita %],
